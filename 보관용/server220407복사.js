...
app.get('/', function(요청, 응답) { 
    ...
    db.collection('post').find().toArray(function(에러, 결과){
        응답.render('index.ejs', {posts : 결과, user: 요청.user});
    });
});
...
app.post('/add', function(요청, 응답){
    ...
    응답.send("<script>alert('할 일 등록 완료'); window.location.replace('/list');</script>");
    //응답.send('전송완료');
...
//나의 할 일 보기에서 완료 버튼 눌렀을 때 글 완료
app.put('/complete', function(요청, 응답){
    요청.body._id = parseInt(요청.body._id); //'1'이라는 문자를 정수로 변환
    //요청.body에 담긴 게시물 번호에 따라 DB에서 게시물 삭제    
    var 완료로변경 = {_id : 요청.body._id, 작성자 : 요청.user._id}

    db.collection('post').updateOne(완료로변경,{ $set : {완료 : true} },function(에러, 결과){
        if(에러){return console.log(에러)};
        console.log(결과);
        console.log('완료로 변경 완료');
        응답.redirect('/mytodo')
    })
});
...
// const 명언 = document.querySelector(".quote__text")
// const 사람 = document.querySelector(".quote__quthor")
